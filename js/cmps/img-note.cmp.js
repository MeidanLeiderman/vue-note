...
export default {
    ...
    template:
        `<section class="note-container flex column space-between" :style="{'background-color': noteToEdit.color || 'white'}" @mouseenter.prevent="isHover=true" @mouseleave="isHover=false">
            <div class="image-note-container">
                <div v-if="noteToEdit.content.text || toEdit" class="text" :contenteditable="toEdit" @focusout="save($event)" ref="text">{{noteToEdit.content.text}}</div>
                <img v-if="noteToEdit.content.url" :src="noteToEdit.content.url"/>
            </div>
    ...
    methods: {
        ...
        save(e) {
            if (this.toEdit) {
                this.noteToEdit.content.text = e.target.textContent
                e.target.textContent = this.noteToEdit.content.text
